...
def format_message(message) -> Dict[str, Any]:
    ...
    if message.media:
        ...
        if hasattr(media, "document") and media.document:
            doc = media.document
            result["media_size"] = getattr(doc, "size", None)
            result["media_mime_type"] = getattr(doc, "mime_type", None)
            if hasattr(doc, "attributes"):
                for attr in doc.attributes:
                    if hasattr(attr, "duration"):
                        result["media_duration"] = attr.duration
                    if hasattr(attr, "round_message"):
                        result["is_round_video"] = attr.round_message
                    if hasattr(attr, "voice") and attr.voice:
                        result["is_voice_message"] = True
                    if hasattr(attr, "alt"):
                        result["sticker_emoji"] = attr.alt
                    attr_name = type(attr).__name__
                    if attr_name == "DocumentAttributeSticker":
                        result["media_subtype"] = "sticker"
                    elif attr_name == "DocumentAttributeAnimated":
                        result["media_subtype"] = "gif"
                    elif attr_name == "DocumentAttributeAudio":
                        result["media_subtype"] = "voice" if getattr(attr, "voice", False) else "audio"
                    elif attr_name == "DocumentAttributeVideo":
                        result["media_subtype"] = "video_note" if getattr(attr, "round_message", False) else "video"
...
async def unarchive_chat(chat_id: Union[int, str]) -> str:
    """
    Unarchive a chat (move back from Archive folder).
    """
    try:
        peer = await client.get_input_entity(chat_id)
        await client(
...
async def run_autoresearch(
    ...
    """
    Run autonomous experiments to improve the ENTIRE project.

    Rotates through 5 experiment types: neural (30%), sklearn (25%),
    rl_params (20%), engine_params (15%), voice (10%). Winning configs
    are auto-promoted and picked up by the live system without restart.

    Args:
        experiment_type: Force specific type ("neural", "sklearn", "rl_params", "engine_params", "voice") or "" for weighted rotation across all
        n_experiments: Number of experiments to run (default: 10)
        budget_seconds: Training budget per experiment in seconds (default: 300)
    """
    ...
    try:
        ...
        return json.dumps(
            {
                "status": "complete",
                "experiments_run": result.get("total", 0),
                "improvements": result.get("wins", 0),
                "wins_by_type": result.get("wins_by_type", {}),
        ...
        return log_and_format_error("run_autoresearch", e)


@mcp.tool()
async def get_autoresearch_status() -> str:
    """
    Get status of autoresearch: model versions, dependencies, and current best scores.
    """
    try:
        from pathlib import Path as _Path

        ar_dir = _Path(__file__).parent / "autoresearch"
        results_file = ar_dir / "results.tsv"
        model_versions_dir = ar_dir / "model_versions"
        ...
        if results_file.exists():
            ...
            status["total_experiments"] = max(0, len(lines) - 1)  # minus header
            # Last 5 results
            for line in lines[-5:]:
                status["latest_results"].append(line)

        if model_versions_dir.exists():
            status["model_versions"] = len(
                [d for d in model_versions_dir.iterdir() if d.is_dir() and d.name.startswith("exp_")]
            )

        # Current production model scores
        trained_dir = _Path(__file__).parent / "trained_models"
        neural_dir = trained_dir / "neural"
        production_models = {}
        if neural_dir.exists():
            for meta_path in neural_dir.glob("*_meta.json"):
                meta = json.loads(meta_path.read_text())
                production_models[meta_path.stem] = meta
        status["production_models"] = production_models

        return json.dumps(status, indent=2, default=str)
    except Exception as e:
        logger.exception("get_autoresearch_status failed")
        return log_and_format_error("get_autoresearch_status", e)


@mcp.tool()
async def get_autoresearch_results(limit: int = 20) -> str:
    """
    Get recent experiment results from autoresearch.

    Args:
        limit: Maximum number of results to return (default: 20)
    """
    try:
        from autoresearch.run_experiment import get_results
        results = get_results(limit=limit)
        return json.dumps(results, indent=2, default=str)
    except Exception as e:
        logger.exception("get_autoresearch_results failed")
        return log_and_format_error("get_autoresearch_results", e)
